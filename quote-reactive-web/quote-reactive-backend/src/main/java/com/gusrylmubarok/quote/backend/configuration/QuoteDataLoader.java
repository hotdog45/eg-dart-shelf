package com.gusrylmubarok.quote.backend.configuration;
...
public class QuoteDataLoader implements ApplicationRunner {
    ...
    public QuoteDataLoader(final QuoteMongoReactiveRepository quoteMongoReactiveRepository) {
        ...
        if(quoteMongoReactiveRepository.count().block() == 0L) {
            ...
            Flux.fromStream(bufferedReader.lines()
                    ...
                    .map(l -> quoteMongoReactiveRepository
                            .save(new Quote(idSupplier.get(), "El Quijote", l))))
